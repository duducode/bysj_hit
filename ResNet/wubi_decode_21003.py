...
tf.app.flags.DEFINE_integer('charset_size', 26, "Choose the first `charset_size` character to conduct our experiment.")
...
tf.app.flags.DEFINE_string('checkpoint_dir1', './checkpoint21003_wubi_1/', 'the checkpoint dir')
tf.app.flags.DEFINE_string('checkpoint_dir2', './checkpoint21003_wubi_2', 'the checkpoint dir')
tf.app.flags.DEFINE_string('checkpoint_dir3', './checkpoint21003_wubi_3/', 'the checkpoint dir')
tf.app.flags.DEFINE_string('checkpoint_dir4', './checkpoint21003_wubi_4/', 'the checkpoint dir')
tf.app.flags.DEFINE_string('log_dir1', './log21003_wubi_1', 'the logging dir')
tf.app.flags.DEFINE_string('log_dir2', './log21003_wubi_2', 'the logging dir')
tf.app.flags.DEFINE_string('log_dir3', './log21003_wubi_3', 'the logging dir')
tf.app.flags.DEFINE_string('log_dir4', './log21003_wubi_4', 'the logging dir')
...
def test():
    ...
    with tf.Session(graph = g1) as sess:
        ...
        graph = build_graph(num_classes=FLAGS.charset_size, top_k=26, is_train=False, is_test=True)
        ...
        while True:
            try:
                ...
                sum_index_top_k = np.zeros([8000000, 26])
                act_index_top_k = np.zeros([8000000, 1])
                sum_pre_val = np.zeros([8000000, 26])
                while not coord.should_stop():
                    i += 1
                    val_images_batch, val_labels_batch = sess.run([val_images, val_labels])
                    for m in range(0, len(val_labels_batch)):
                        val_labels_batch[m] = dict[line[val_labels_batch[m] + 1][2:3]]
    ...
    with tf.Session(graph = g2) as sess:
        ...
        graph2 = build_graph(num_classes=FLAGS.charset_size, top_k=26, is_train=False, is_test=True)
        saver = tf.train.Saver()
        sess.run(tf.global_variables_initializer())
        sess.run(tf.local_variables_initializer())
        if FLAGS.restore:
        ...
        while True:
            try:
                ...
                sum_index_top_k2 = np.zeros([8000000, 26])
                act_index_top_k2 = np.zeros(8000000)
                sum_pre_val2 = np.zeros([8000000, 26])
                while not coord.should_stop():
                    i += 1
                    val_images_batch, val_labels_batch = sess.run([val_images2, val_labels2])
                    for m in range(0, len(val_labels_batch)):
                        val_labels_batch[m] = dict[line[val_labels_batch[m] + 1][3:4]]
    ...
    with tf.Session(graph = g3) as sess:
        ...
        graph3 = build_graph(num_classes=FLAGS.charset_size, top_k=26, is_train=False, is_test=True)
        saver = tf.train.Saver()
        sess.run(tf.global_variables_initializer())
        sess.run(tf.local_variables_initializer())
        if FLAGS.restore:
        ...
        while True:
            try:
                ...
                sum_index_top_k3 = np.zeros([8000000, 26])
                act_index_top_k3 = np.zeros(8000000)
                sum_pre_val3 = np.zeros([8000000, 26])
                while not coord.should_stop():
                    i += 1
                    val_images_batch, val_labels_batch = sess.run([val_images3, val_labels3])
                    for m in range(0, len(val_labels_batch)):
                        val_labels_batch[m] = dict[line[val_labels_batch[m] + 1][4:5]]
    ...
    with tf.Session(graph = g4) as sess:
        ...
        graph4 = build_graph(num_classes=FLAGS.charset_size, top_k=26, is_train=False, is_test=True)
        saver = tf.train.Saver()
        sess.run(tf.global_variables_initializer())
        sess.run(tf.local_variables_initializer())
        if FLAGS.restore:
        ...
        while True:
            try:
                ...
                sum_index_top_k4 = np.zeros([8000000, 26])
                act_index_top_k4 = np.zeros(8000000)
                sum_pre_val4 = np.zeros([8000000, 26])
                while not coord.should_stop():
                    i += 1
                    val_images_batch, val_labels_batch = sess.run([val_images4, val_labels4])
                    for m in range(0, len(val_labels_batch)):
                        val_labels_batch[m] = dict[line[val_labels_batch[m] + 1][5:6]]
    ...
    for i in range(0, batch_num):
        ...
        for j in range(0, 21003):
            a = np.argwhere(index67_batch == dict[line[j + 1][2:3]])
            b = np.argwhere(index71_batch == dict[line[j + 1][3:4]])
            c = np.argwhere(index73_batch == dict[line[j + 1][4:5]])
            d = np.argwhere(index79_batch == dict[line[j + 1][5:6]])
        ...
        # print('index1:', index67_batch[0])
        # print('index2:', index71_batch[0])
        # print('index3:', index73_batch[0])
        # print('index4:', index79_batch[0])
        # print('pro4:', num4[0])
        #ar1 = num2 % 67
        ar6 = np.zeros(10000)
        ar7 = np.zeros(10000)
        ...
        for o in range(0, 10000):
            ar6[o] = dict[line[num4[o] + 1][2:3]]
            ar7[o] = dict[line[num4[o] + 1][3:4]]
            ar8[o] = dict[line[num4[o] + 1][4:5]]
            ar9[o] = dict[line[num4[o] + 1][5:6]]
        # print(ar6[0])
        # print(ar7[0])
        # print(ar8[0])
        # print(ar9[0])
