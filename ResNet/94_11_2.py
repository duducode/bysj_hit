...
tf.app.flags.DEFINE_integer('decay_steps', 222, '')
    ...
    with sess:
        ...
        while True:
            try:
                ...
                train_labels_batch = ((train_labels_batch % 11) + np.ceil(train_labels_batch / 11) * 2) % 11
                #train_labels_batch = train_labels_batch % 13
                ...
                if step % FLAGS.eval_steps == 0:
                    ...
                    logger.info("@ Accuracy of train set util {0}: {1}/{2}".format(step, allAcc1Train, allAcc10Train))
                    allAcc1Train = 0.0
                    allAcc10Train = 0.0
                    trainIter = 0.0
                    ...
                    # logger.info("@ Validation start time: {0}".format(datetime.now()))
                    while True:
                        try:
                            i = 0
                            acc_top_1 = 0.0
                            ...
                            while not coord.should_stop():
                                i += 1
                                val_images_batch, val_labels_batch = sess.run([val_images, val_labels])
                                val_labels_batch = ((val_labels_batch % 11) + np.ceil(val_labels_batch / 11) * 2) % 11
                                #val_labels_batch = val_labels_batch % 13
                                feed_dict = {vali_graph['images']: val_images_batch,
                                             vali_graph['labels']: val_labels_batch
                            ...
                            logger.info('epoch # {0}, top 1 accuracy: {1}, top 5 accuracy: {2}'.format(num, acc_top_1,
                                                                                                        acc_top_10))
                    ...
                    if maxAcc < acc[num]:
                        ...
                        count = count + 1
                        print('count=', count)
                        logger.info('@ Early stop num: {0}'.format(count))
                    # logger.info('@ Save the ckpt of {0} step(s)/{1} epoch(es)'.format(step, num ))
                    num = num + 1
                    # saver.save(sess, os.path.join(FLAGS.checkpoint_dir, 'my-model'),
                    #            global_step=graph['global_step'])
                ...
                if step > FLAGS.max_steps:
                    logger.info('@ Best Accuracy: {0} (Step over)'.format(maxAcc))
                    break
                if count == 10:
                    logger.info('@ Best Accuracy: {0} (Early stopped)'.format(maxAcc))
                    break
            except tf.errors.OutOfRangeError:
                logger.info('@ Best Accuracy: {0} (Out of range) step : {1}'.format(maxAcc, step))
                break
    train_writer.close()
...
def inference(image):
    ...
    with tf.Session() as sess:
        ...
        start_time = time.time()
        graph = build_graph(num_classes=FLAGS.charset_size, top_k=10, is_train=False, is_test=True)
