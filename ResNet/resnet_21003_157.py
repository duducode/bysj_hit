...
tf.app.flags.DEFINE_integer('charset_size', 157, "Choose the first `charset_size` character to conduct our experiment.")
...
tf.app.flags.DEFINE_string('checkpoint_dir', './checkpoint21003_157/', 'the checkpoint dir')
...
tf.app.flags.DEFINE_string('log_dir', './log21003_157', 'the logging dir')
    ...
    with sess:
        ...
        if FLAGS.restore:
            ...
            if ckpt:
                ...
                print("Resnet21003_157 restore from the checkpoint {0}".format(ckpt))
        ...
        while True:
            try:
                ...
                train_labels_batch = train_labels_batch % 157
                ...
                if step % FLAGS.eval_steps == 0:
                    ...
                    logger.info("@Resnet21003_157 Accuracy of train set util {0}: {1}/{2}".format(step, allAcc1Train, allAcc10Train))
                    allAcc1Train = 0.0
                    allAcc10Train = 0.0
                    trainIter = 0.0
                    ...
                    while True:
                        try:
                            ...
                            logger.info('Resnet21003_157 epoch # {0}, top 1 accuracy: {1}, top 5 accuracy: {2}'.format(num, acc_top_1,
                                                                                                        acc_top_10))
                    ...
                    if maxAcc < acc[num]:
                        ...
                        logger.info('@Resnet21003_157 Save the ckpt of {0} step(s)/{1} epoch(es)'.format(step, num))
                        saver.save(sess, os.path.join(FLAGS.checkpoint_dir, 'my-model'),
                                   global_step=graph['global_step'])
                    else:
                        count = count + 1
                        print('count=', count)
                        logger.info('@Resnet21003_157 Early stop num: {0}'.format(count))
                ...
                if step > FLAGS.max_steps:
                    logger.info('@Resnet21003_157 Best Accuracy: {0} (Step over)'.format(maxAcc))
                    break
                if count == 10:
                    logger.info('@Resnet21003_157 Best Accuracy: {0} (Early stopped)'.format(maxAcc))
                    break
            except tf.errors.OutOfRangeError:
                logger.info('@Resnet21003_157 Best Accuracy: {0} (Out of range) step : {1}'.format(maxAcc, step))
                break
    train_writer.close()
