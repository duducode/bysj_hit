...
tf.app.flags.DEFINE_integer('charset_size2', 67, "Choose the first `charset_size` character to conduct our experiment.")
tf.app.flags.DEFINE_integer('charset_size3', 67, "Choose the first `charset_size` character to conduct our experiment.")
tf.app.flags.DEFINE_integer('charset_size4', 67, "Choose the first `charset_size` character to conduct our experiment.")
...
tf.app.flags.DEFINE_string('checkpoint_dir1', './checkpoint3755_67_1/', 'the checkpoint dir')
tf.app.flags.DEFINE_string('checkpoint_dir2', './checkpoint3755_71_2/', 'the checkpoint dir')
tf.app.flags.DEFINE_string('checkpoint_dir3', './checkpoint3755_73_3/', 'the checkpoint dir')
tf.app.flags.DEFINE_string('checkpoint_dir4', './checkpoint3755_79_4/', 'the checkpoint dir')
tf.app.flags.DEFINE_string('log_dir1', './log3755_67_1', 'the logging dir')
tf.app.flags.DEFINE_string('log_dir2', './log3755_71_2', 'the logging dir')
tf.app.flags.DEFINE_string('log_dir3', './log3755_73_3', 'the logging dir')
tf.app.flags.DEFINE_string('log_dir4', './log3755_79_4', 'the logging dir')
...
tf.app.flags.DEFINE_string('val_data_dir', '/home/why2/dataset/3755/al', 'the val dataset dir')
...
def test():
    ...
    with tf.Session(graph = g1) as sess:
        ...
        while True:
            try:
                ...
                sum_index_top_k = np.zeros([8000000, 67])
                act_index_top_k = np.zeros([8000000, 1])
                sum_pre_val = np.zeros([8000000, 67])
                while not coord.should_stop():
                    i += 1
                    val_images_batch, val_labels_batch = sess.run([val_images, val_labels])
                    val_labels_batch = ((val_labels_batch % 67) + np.ceil(val_labels_batch / 67) * 1) % 67
    ...
    with tf.Session(graph = g2) as sess:
        ...
        graph2 = build_graph(num_classes=FLAGS.charset_size2, top_k=67, is_train=False, is_test=True)
        saver = tf.train.Saver()
        sess.run(tf.global_variables_initializer())
        sess.run(tf.local_variables_initializer())
        if FLAGS.restore:
        ...
        while True:
            try:
                ...
                sum_index_top_k2 = np.zeros([8000000, 67])
                act_index_top_k2 = np.zeros(8000000)
                sum_pre_val2 = np.zeros([8000000, 67])
                while not coord.should_stop():
                    i += 1
                    val_images_batch, val_labels_batch = sess.run([val_images2, val_labels2])
                    val_labels_batch = ((val_labels_batch % 67) + np.ceil(val_labels_batch / 67) * 2) % 67
    ...
    with tf.Session(graph = g3) as sess:
        ...
        graph3 = build_graph(num_classes=FLAGS.charset_size3, top_k=67, is_train=False, is_test=True)
        saver = tf.train.Saver()
        sess.run(tf.global_variables_initializer())
        sess.run(tf.local_variables_initializer())
        if FLAGS.restore:
        ...
        while True:
            try:
                ...
                sum_index_top_k3 = np.zeros([8000000, 67])
                act_index_top_k3 = np.zeros(8000000)
                sum_pre_val3 = np.zeros([8000000, 67])
                while not coord.should_stop():
                    i += 1
                    val_images_batch, val_labels_batch = sess.run([val_images3, val_labels3])
                    val_labels_batch = ((val_labels_batch % 67) + np.ceil(val_labels_batch / 67) * 3) % 67
    ...
    with tf.Session(graph = g4) as sess:
        ...
        graph4 = build_graph(num_classes=FLAGS.charset_size4, top_k=67, is_train=False, is_test=True)
        saver = tf.train.Saver()
        sess.run(tf.global_variables_initializer())
        sess.run(tf.local_variables_initializer())
        if FLAGS.restore:
        ...
        while True:
            try:
                ...
                sum_index_top_k4 = np.zeros([8000000, 67])
                act_index_top_k4 = np.zeros(8000000)
                sum_pre_val4 = np.zeros([8000000, 67])
                while not coord.should_stop():
                    i += 1
                    val_images_batch, val_labels_batch = sess.run([val_images4, val_labels4])
                    val_labels_batch = ((val_labels_batch % 67) + np.ceil(val_labels_batch / 67) * 4) % 67
    ...
    for i in range(0, batch_num):
        ...
        pro2 = np.zeros([batch_size, 3755])
        pro3 = np.zeros([batch_size, 3755])
        pro4 = np.zeros([batch_size, 3755])

        for j in range(0, 3755):
            a = np.argwhere(index67_batch == (((j % 67) + np.ceil(j / 67) * 1) % 67))
            b = np.argwhere(index71_batch == (((j % 67) + np.ceil(j / 67) * 2) % 67))
            c = np.argwhere(index73_batch == (((j % 67) + np.ceil(j / 67) * 3) % 67))
            d = np.argwhere(index79_batch == (((j % 67) + np.ceil(j / 67) * 4) % 67))
        ...
        ((val_labels_batch % 67) + np.ceil(val_labels_batch / 67) * 4) % 67

        ar1 = ((num2 % 67) + np.ceil(num2 / 67) * 1) % 67
        ar2 = ((num2 % 67) + np.ceil(num2 / 67) * 2) % 67
        ar3 = ((num3 % 67) + np.ceil(num3 / 67) * 1) % 67
        ar4 = ((num3 % 67) + np.ceil(num3 / 67) * 2) % 67
        ar5 = ((num3 % 67) + np.ceil(num3 / 67) * 3) % 67
        ar6 = ((num4 % 67) + np.ceil(num4 / 67) * 1) % 67
        ar7 = ((num4 % 67) + np.ceil(num4 / 67) * 2) % 67
        ar8 = ((num4 % 67) + np.ceil(num4 / 67) * 3) % 67
        ar9 = ((num4 % 67) + np.ceil(num4 / 67) * 4) % 67
