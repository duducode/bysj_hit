...
tf.app.flags.DEFINE_integer('image_size', 112, "Nee ds to provide same value as in training.")
...
tf.app.flags.DEFINE_string('checkpoint_dir', './checkpoint3755_67_4/', 'the checkpoint dir')
...
tf.app.flags.DEFINE_string('log_dir', './log3755_67_4', 'the logging dir')
    ...
    with sess:
        ...
        if FLAGS.restore:
            ...
            if ckpt:
                ...
                print("Resnet3755_671 restore from the checkpoint {0}".format(ckpt))
        ...
        while True:
            try:
                ...
                train_labels_batch = ((train_labels_batch%67)+np.ceil(train_labels_batch/67)*1)%67 
                ...
                if step % FLAGS.eval_steps == 0:
                    ...
                    logger.info("@Resnet3755_671 Accuracy of train set util {0}: {1}/{2}".format(step, allAcc1Train, allAcc10Train))
                    allAcc1Train = 0.0
                    allAcc10Train = 0.0
                    trainIter = 0.0
                    ...
                    while True:
                        try:
                            ...
                            logger.info('Resnet3755_671 epoch # {0}, top 1 accuracy: {1}, top 5 accuracy: {2}'.format(num, acc_top_1,
                                                                                                        acc_top_10))
                    ...
                    if maxAcc < acc[num]:
                        ...
                        logger.info('@Resnet3755_671 Save the ckpt of {0} step(s)/{1} epoch(es)'.format(step, num))
                        saver.save(sess, os.path.join(FLAGS.checkpoint_dir, 'my-model'),
                                   global_step=graph['global_step'])
                    else:
                        count = count + 1
                        print('count=', count)
                        logger.info('@Resnet3755_671 Early stop num: {0}'.format(count))
                ...
                if step > FLAGS.max_steps:
                    logger.info('@Resnet3755_671 Best Accuracy: {0} (Step over)'.format(maxAcc))
                    break
                if count == 10:
                    logger.info('@Resnet3755_671 Best Accuracy: {0} (Early stopped)'.format(maxAcc))
                    break
            except tf.errors.OutOfRangeError:
                logger.info('@Resnet3755_671 Best Accuracy: {0} (Out of range) step : {1}'.format(maxAcc, step))
                break
    train_writer.close()
