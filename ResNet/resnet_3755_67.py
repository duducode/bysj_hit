    ...
    with sess:
        ...
        if FLAGS.restore:
            ...
            if ckpt:
                ...
                print("Resnet3755_67 restore from the checkpoint {0}".format(ckpt))
        ...
        while True:
            try:
                ...
                if step % FLAGS.eval_steps == 0:
                    ...
                    logger.info("@Resnet3755_67 Accuracy of train set util {0}: {1}/{2}".format(step, allAcc1Train, allAcc10Train))
                    allAcc1Train = 0.0
                    allAcc10Train = 0.0
                    trainIter = 0.0
                    ...
                    while True:
                        try:
                            ...
                            logger.info('Resnet3755_67 epoch # {0}, top 1 accuracy: {1}, top 5 accuracy: {2}'.format(num, acc_top_1,
                                                                                                        acc_top_10))
                    ...
                    if maxAcc < acc[num]:
                        ...
                        logger.info('@Resnet3755_67 Save the ckpt of {0} step(s)/{1} epoch(es)'.format(step, num))
                        saver.save(sess, os.path.join(FLAGS.checkpoint_dir, 'my-model'),
                                   global_step=graph['global_step'])
                    else:
                        count = count + 1
                        print('count=', count)
                        logger.info('@Resnet3755_67 Early stop num: {0}'.format(count))
                ...
                if step > FLAGS.max_steps:
                    logger.info('@Resnet3755_67 Best Accuracy: {0} (Step over)'.format(maxAcc))
                    break
                if count == 10:
                    logger.info('@Resnet3755_67 Best Accuracy: {0} (Early stopped)'.format(maxAcc))
                    break
            except tf.errors.OutOfRangeError:
                logger.info('@Resnet3755_67 Best Accuracy: {0} (Out of range) step : {1}'.format(maxAcc, step))
                break
    train_writer.close()
    ...
    with tf.Session() as sess:
        ...
        while True:
            try:
                ...
                sum_index_top_k = np.zeros([800000, 68])
                sum_pre_val = np.zeros([800000, 67])
                while not coord.should_stop():
                    i += 1
                    val_images_batch, val_labels_batch = sess.run([val_images, val_labels])
                    val_labels_batch = val_labels_batch % 67
                    ...
                    #print('pre_label:', index_top_k)
                    #print('val:', pre_val)
                    #print('labels', act_label[0])
                    # a = np.zeros((67))
                    # a[0] = index_top_k[0]
                    # b = np.array([0.0])
                    # b[0] = pre_val[0]
                    ...
                    for n in range(0, len(act_label)):
                        ...
                        #sum_pre_val[n + (i - 1) * 256][67] = act_label[n]
                    # if i == 1:
                    #     print(sum_index_top_k[0])
                    #     print(sum_pre_val[0])
                    #     print(sum_index_top_k[255])
                    #     print(sum_pre_val[255])
                    if i % 100 == 0:
                ...
                # print(len(sum_index_top_k))
                # print(len(sum_pre_val))
                np.savetxt('67_val_index.txt', sum_index_top_k, fmt='%d')
                np.savetxt('67_val_result.txt', sum_pre_val, fmt='%1.4e')
                break
