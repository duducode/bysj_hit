...
tf.app.flags.DEFINE_integer('decay_steps', 10625, '')
    ...
    with sess:
        ...
        if FLAGS.restore:
            ...
            if ckpt:
                ...
                print("Resnet3755_71 restore from the checkpoint {0}".format(ckpt))
        ...
        while True:
            try:
                ...
                train_labels_batch = train_labels_batch % 73
                ...
                if step % FLAGS.eval_steps == 0:
                    ...
                    logger.info("@Resnet3755_71 Accuracy of train set util {0}: {1}/{2}".format(step, allAcc1Train, allAcc10Train))
                    allAcc1Train = 0.0
                    allAcc10Train = 0.0
                    trainIter = 0.0
                    ...
                    while True:
                        try:
                            ...
                            logger.info('Resnet3755_71 epoch # {0}, top 1 accuracy: {1}, top 5 accuracy: {2}'.format(num, acc_top_1,
                                                                                                        acc_top_10))
                    ...
                    if maxAcc < acc[num]:
                        ...
                        logger.info('@Resnet3755_71 Save the ckpt of {0} step(s)/{1} epoch(es)'.format(step, num))
                        saver.save(sess, os.path.join(FLAGS.checkpoint_dir, 'my-model'),
                                   global_step=graph['global_step'])
                    else:
                        count = count + 1
                        print('count=', count)
                        logger.info('@Resnet3755_71 Early stop num: {0}'.format(count))
                ...
                if step > FLAGS.max_steps:
                    logger.info('@Resnet3755_71 Best Accuracy: {0} (Step over)'.format(maxAcc))
                    break
                if count == 10:
                    logger.info('@Resnet3755_71 Best Accuracy: {0} (Early stopped)'.format(maxAcc))
                    break
            except tf.errors.OutOfRangeError:
                logger.info('@Resnet3755_71 Best Accuracy: {0} (Out of range) step : {1}'.format(maxAcc, step))
                break
    train_writer.close()
...
def validation():
    ...
    with tf.Session() as sess:
        ...
        graph = build_graph(num_classes=FLAGS.charset_size, top_k=73, is_train=False, is_test=True)
        saver = tf.train.Saver()
        sess.run(tf.global_variables_initializer())
        sess.run(tf.local_variables_initializer())
        ...
        while True:
            try:
                ...
                sum_index_top_k = np.zeros([770000, 74])
                sum_pre_val = np.zeros([770000, 73])
                while not coord.should_stop():
                    i += 1
                    val_images_batch, val_labels_batch = sess.run([val_images, val_labels])
                    val_labels_batch = val_labels_batch % 73
                    ...
                    for n in range(0, len(act_label)):
                        #print(act_label[n])
                        #print(index_top_k[n])
                        # print(act_label[n] +index_top_k[n])
                        #print(np.append(act_label[n], index_top_k[n]))
                ...
                # a[0] = acc_top_1
                # a[1] = acc_top_k
                # np.savetxt('67_val_result.txt', a, fmt='%1.4e')
                # print(i)
                # print(len(sum_index_top_k))
                # print(len(sum_pre_val))
                # for n in range(0, len(sum_index_top_k)):
                #     if (sum_index_top_k[n][0]==0) & (sum_index_top_k[n][1]==0) & (sum_index_top_k[n][2]==0):
                #         sum_index_top_k = np.delete(sum_index_top_k, n, axis=0)
                #         sum_pre_val = np.delete(sum_pre_val, n, axis=0)
                #         print('delete:', n)
                # print(len(sum_index_top_k))
                # print(len(sum_pre_val))
                np.savetxt('73_val_index.txt', sum_index_top_k, fmt='%d')
                np.savetxt('73_val_result.txt', sum_pre_val, fmt='%1.4e')
                break
        return sum_index_top_k, sum_pre_val
...
def main(_):
    ...
    if FLAGS.mode == "train":
        ...
        np.savetxt('71_index.txt', final_predict_index, fmt='%d')
        np.savetxt('71_result.txt', final_predict_val, fmt='%1.4e')
