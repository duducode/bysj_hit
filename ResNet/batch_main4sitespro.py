...
def test():
    ...
    with tf.Session(graph = g2) as sess:
        ...
        while True:
            try:
                ...
                sum_index_top_k2 = np.zeros([8000000, 71])
    ...
    with tf.Session(graph = g3) as sess:
        ...
        while True:
            try:
                ...
                sum_index_top_k3 = np.zeros([8000000, 73])
    ...
    with tf.Session(graph = g4) as sess:
        ...
        while True:
            try:
                ...
                sum_index_top_k4 = np.zeros([8000000, 79])
    ...
    for i in range(0, batch_num):
        ...
        for j in range(0, 3755):
            a = np.argwhere(index67_batch == (j % 67))
            b = np.argwhere(index71_batch == (j % 71))
            c = np.argwhere(index73_batch == (j % 73))
            d = np.argwhere(index79_batch == (j % 79))
