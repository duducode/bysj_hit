...
tf.app.flags.DEFINE_integer('decay_steps', 222, "the steps to save")
...
tf.app.flags.DEFINE_string('png_dir', 'H:/苏统华的空间/3755类汉字样本for荟俨/2/247.png', '')
    ...
    with sess:
        ...
        if FLAGS.restore:
            ...
            if ckpt:
                ...
                print("Resnet3755 restore from the checkpoint {0}".format(ckpt))
        ...
        while True:
            try:
                ...
                if step % FLAGS.eval_steps == 0:
                    ...
                    logger.info("@Resnet3755 Accuracy of train set util {0}: {1}/{2}".format(step, allAcc1Train, allAcc10Train))
                    allAcc1Train = 0.0
                    allAcc10Train = 0.0
                    trainIter = 0.0
                    ...
                    while True:
                        try:
                            ...
                            logger.info('Resnet3755 epoch # {0}, top 1 accuracy: {1}, top 5 accuracy: {2}'.format(num, acc_top_1,
                                                                                                        acc_top_10))
                    ...
                    if maxAcc < acc[num]:
                        ...
                        logger.info('@Resnet3755 Save the ckpt of {0} step(s)/{1} epoch(es)'.format(step, num))
                        saver.save(sess, os.path.join(FLAGS.checkpoint_dir, 'my-model'),
                                   global_step=graph['global_step'])
                    else:
                        count = count + 1
                        print('count=', count)
                        logger.info('@Resnet3755 Early stop num: {0}'.format(count))
                ...
                if step > FLAGS.max_steps:
                    logger.info('@Resnet3755 Best Accuracy: {0} (Step over)'.format(maxAcc))
                    break
                if count == 10:
                    logger.info('@Resnet3755 Best Accuracy: {0} (Early stopped)'.format(maxAcc))
                    break
            except tf.errors.OutOfRangeError:
                logger.info('@Resnet3755 Best Accuracy: {0} (Out of range) step : {1}'.format(maxAcc, step))
                break
    train_writer.close()
        ...
        while True:
            try:
                ...
                while not coord.should_stop():
                    i += 1
                    val_images_batch, val_labels_batch = sess.run([val_images, val_labels])
                    #val_labels_batch = val_labels_batch % 79
