...
def test():
    ...
    with tf.Session(graph = g1) as sess:
        ...
        while True:
            try:
                ...
                sum_index_top_k = np.zeros([8000000, 150])
                ...
                with open("67_result.txt", "w") as f:
                    f.write('67:'+str(acc_top_1)+' '+str(acc_top_k))
                break
    count = 0
    ...
    with tf.Session(graph = g2) as sess:
        ...
        while True:
            try:
                ...
                sum_index_top_k2 = np.zeros([8000000, 152])
                ...
                with open("67_result.txt", "w") as f:
                    f.write('71:'+str(acc_top_12)+' '+str(acc_top_k2))
                break
    ...
    with tf.Session(graph=g3) as sess:
        ...
        while True:
            try:
                ...
                sum_index_top_k3 = np.zeros([8000000, 158])
                ...
                with open("67_result.txt", "w") as f:
                    f.write('73:'+str(acc_top_13)+' '+str(acc_top_k3))
                break
    ...
    with tf.Session(graph=g4) as sess:
        ...
        while True:
            try:
                ...
                sum_index_top_k4 = np.zeros([8000000, 164])
                ...
                while not coord.should_stop():
                    ...
                    for n in range(0, len(act_label4)):
                        sum_index_top_k4[n + (i - 1) * 256] = np.append(act_label4[n], index_top_k4[n])
                ...
                with open("67_result.txt", "w") as f:
                    f.write('79:'+str(acc_top_14)+' '+str(acc_top_k4))
                break
    ...
    for i in range(0, count):
        ...
        for j in range(0, 21003):
            if(len(np.argwhere(index_67[i] == j % 149)) == 2):
                a = np.argwhere(index_67[i] == j % 149)[1][0]
            else:
                a = np.argwhere(index_67[i] == j % 149)[0][0]

            if (len(np.argwhere(index_71[i] == j % 151)) == 2):
                b = np.argwhere(index_71[i] == j % 151)[1][0]
            else:
                b = np.argwhere(index_71[i] == j % 151)[0][0]

            if (len(np.argwhere(index_73[i] == j % 157)) == 2):
                c = np.argwhere(index_73[i] == j % 157)[1][0]
            else:
                c = np.argwhere(index_73[i] == j % 157)[0][0]

            if (len(np.argwhere(index_79[i] == j % 163)) == 2):
                d = np.argwhere(index_79[i] == j % 163)[1][0]
            else:
                d = np.argwhere(index_79[i] == j % 163)[0][0]
            pro2[j] = math.log(result_67[i][a-1] + 1e-50) + math.log(result_71[i][b-1] + 1e-50)
            pro3[j] = math.log(result_67[i][a-1] + 1e-50) + math.log(result_71[i][b-1] + 1e-50) + math.log(result_73[i][c-1]+1e-50)
            pro4[j] = math.log(result_67[i][a-1]+1e-50) + math.log(result_71[i][b-1]+1e-50) + math.log(result_73[i][c-1]+1e-50) + math.log(result_79[i][d-1]+1e-50)
        num2 = np.where(pro2 == np.max(pro2))[0][0]
        num3 = np.where(pro3 == np.max(pro3))[0][0]
        num4 = np.where(pro4 == np.max(pro4))[0][0]
        if (num2 % 149 ==index_67[i][0])&(num2 % 151 ==index_71[i][0]):
            sum_acc2 += 1.0
        if (num3 % 149 ==index_67[i][0])&(num3 % 151 ==index_71[i][0])&(num3 % 157 ==index_73[i][0]):
            sum_acc3 += 1.0
        if (num4 % 139 ==index_67[i][0])&(num4 % 151 ==index_71[i][0])&(num4 % 157 ==index_73[i][0])&(num4 % 163 ==index_79[i][0]):
            sum_acc4 += 1.0
        #else:
