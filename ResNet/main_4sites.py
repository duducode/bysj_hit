...
def test():
    ...
    index_79 = np.loadtxt('79_val_index.txt')
    sum_acc = 0.0
    for i in range(0, 760319):
        ...
        for j in range(0, 3755):
            ...
            pro[j] = math.log(result_67[i][a-1]) + math.log(result_71[i][b-1]) + math.log(result_73[i][c-1]) + math.log(result_79[i][d-1])
        num = np.where(pro == np.max(pro))[0][0]
        if (num % 67 ==index_67[i][0])&(num % 71 ==index_71[i][0])&(num % 73 ==index_73[i][0])&(num % 79 ==index_79[i][0]):
            sum_acc += 1.0
        else:
