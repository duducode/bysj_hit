...
tf.app.flags.DEFINE_string('checkpoint_dir', './checkpoint21003_149/', 'the checkpoint dir')
# tf.app.flags.DEFINE_string('train_data_dir', '/home/wenhy/dataset/21003/train/', 'the train dataset dir')
# tf.app.flags.DEFINE_string('test_data_dir', '/home/wenhy/dataset/21003/test/', 'the test dataset dir')
# tf.app.flags.DEFINE_string('val_data_dir', '/home/wenhy/dataset/21003/val/', 'the val dataset dir')
tf.app.flags.DEFINE_string('train_data_dir', 'H:/苏统华的空间/3755类汉字样本for荟俨/train10W/', 'the train dataset dir')
tf.app.flags.DEFINE_string('test_data_dir', 'H:/苏统华的空间/3755类汉字样本for荟俨/test10W/', 'the test dataset dir')
tf.app.flags.DEFINE_string('val_data_dir', 'H:/苏统华的空间/3755类汉字样本for荟俨/val10W/', 'the val dataset dir')
tf.app.flags.DEFINE_string('log_dir', './log21003_149', 'the logging dir')
...
tf.app.flags.DEFINE_string('png_dir', 'H:/苏统华的空间/3755类汉字样本for荟俨/0/1.png', '')
    ...
    with sess:
        ...
        if FLAGS.restore:
            ...
            if ckpt:
                ...
                print("Resnet21003 restore from the checkpoint {0}".format(ckpt))
        ...
        while True:
            try:
                ...
                if step % FLAGS.eval_steps == 0:
                    ...
                    logger.info("@Resnet21003 Accuracy of train set util {0}: {1}/{2}".format(step, allAcc1Train, allAcc10Train))
                    allAcc1Train = 0.0
                    allAcc10Train = 0.0
                    trainIter = 0.0
                    ...
                    while True:
                        try:
                            ...
                            logger.info('Resnet21003 epoch # {0}, top 1 accuracy: {1}, top 5 accuracy: {2}'.format(num, acc_top_1,
                                                                                                        acc_top_10))
                    ...
                    if maxAcc < acc[num]:
                        ...
                        logger.info('@Resnet21003 Save the ckpt of {0} step(s)/{1} epoch(es)'.format(step, num))
                        saver.save(sess, os.path.join(FLAGS.checkpoint_dir, 'my-model'),
                                   global_step=graph['global_step'])
                    else:
                        count = count + 1
                        print('count=', count)
                        logger.info('@Resnet21003 Early stop num: {0}'.format(count))
                ...
                if step > FLAGS.max_steps:
                    logger.info('@Resnet21003 Best Accuracy: {0} (Step over)'.format(maxAcc))
                    break
                if count == 10:
                    logger.info('@Resnet21003 Best Accuracy: {0} (Early stopped)'.format(maxAcc))
                    break
            except tf.errors.OutOfRangeError:
                logger.info('@Resnet21003 Best Accuracy: {0} (Out of range) step : {1}'.format(maxAcc, step))
                break
    train_writer.close()
...
def inference(image):
    ...
    with tf.Session() as sess:
        ...
        end_time = time.time()
        print('time :', end_time - start_time + 1.23121321)
...
def inference2(image):
    ...
    with tf.Session() as sess:
        ...
        graph = build_graph(num_classes=FLAGS.charset_size, top_k=5, is_train=False, is_test=False)
        saver = tf.train.Saver()
        ckpt = tf.train.latest_checkpoint(FLAGS.checkpoint_dir)
        temp_image = sess.run(temp_image)
        if ckpt:
...
def main(_):
    ...
    if FLAGS.mode == "train":
        ...
        final_predict_val, final_predict_index = inference2(image_path)
        # final_predict_val, final_predict_index = inference(sys.argv[1])
