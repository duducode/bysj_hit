...
tf.app.flags.DEFINE_string('checkpoint_dir', './checkpoint94_1/', 'the checkpoint dir')
...
tf.app.flags.DEFINE_string('log_dir', './log94_1', 'the logging dir')

tf.app.flags.DEFINE_boolean('restore', True, 'whether to restore from checkpoint')
tf.app.flags.DEFINE_integer('epoch', 15, 'Number of epoches')
tf.app.flags.DEFINE_integer('decay_steps', 111, '')
...
def build_graph(num_classes=FLAGS.charset_size, top_k=5, is_train=True, is_test=False):
    ...
    rate = tf.train.exponential_decay(0.0001, global_step, decay_steps=FLAGS.decay_steps, decay_rate=0.97,
                                      staircase=True)
    ...
    with sess:
        ...
        if FLAGS.restore:
            ...
            if ckpt:
                ...
                print("Resnet94_1 restore from the checkpoint {0}".format(ckpt))
        ...
        while True:
            try:
                ...
                train_labels_batch = ((train_labels_batch % 11) + np.ceil(train_labels_batch / 11) * 1) % 11
                ...
                if step % FLAGS.eval_steps == 0:
                    ...
                    logger.info("@Resnet94_1 Accuracy of train set util {0}: {1}/{2}".format(step, allAcc1Train, allAcc10Train))
                    allAcc1Train = 0.0
                    allAcc10Train = 0.0
                    trainIter = 0.0
                    ...
                    while True:
                        try:
                            ...
                            while not coord.should_stop():
                                i += 1
                                val_images_batch, val_labels_batch = sess.run([val_images, val_labels])
                                val_labels_batch = ((val_labels_batch % 11) + np.ceil(val_labels_batch / 11) * 1) % 11
                                feed_dict = {vali_graph['images']: val_images_batch,
                                             vali_graph['labels']: val_labels_batch
                            ...
                            logger.info('Resnet94_1 epoch # {0}, top 1 accuracy: {1}, top 5 accuracy: {2}'.format(num, acc_top_1,
                                                                                                        acc_top_10))
                    ...
                    if maxAcc < acc[num]:
                        ...
                        logger.info('@Resnet94_1 Save the ckpt of {0} step(s)/{1} epoch(es)'.format(step, num))
                        saver.save(sess, os.path.join(FLAGS.checkpoint_dir, 'my-model'),
                                   global_step=graph['global_step'])
                    else:
                        count = count + 1
                        print('count=', count)
                        logger.info('@Resnet94_1 Early stop num: {0}'.format(count))
                ...
                if step > FLAGS.max_steps:
                    logger.info('@Resnet94_1 Best Accuracy: {0} (Step over)'.format(maxAcc))
                    break
                if count == 10:
                    logger.info('@Resnet94_1 Best Accuracy: {0} (Early stopped)'.format(maxAcc))
                    break
            except tf.errors.OutOfRangeError:
                logger.info('@Resnet94_1 Best Accuracy: {0} (Out of range) step : {1}'.format(maxAcc, step))
                break
    train_writer.close()
...
def main(_):
    ...
    if FLAGS.mode == "train":
        ...
        np.savetxt('67_index.txt', final_predict_index, fmt='%d')
        np.savetxt('67_result.txt', final_predict_val, fmt='%1.4e')
