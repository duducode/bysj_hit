...
def build_graph2(num_classes=FLAGS.charset_size, top_k=5, is_train=True, is_test=False):
    images = tf.placeholder(dtype=tf.float32, shape=[None, FLAGS.image_size, FLAGS.image_size, 1], name='image_batch')
    labels = tf.placeholder(dtype=tf.int64, shape=[None], name='label_batch')
    # is_training = tf.placeholder(dtype=tf.bool, name='training_mode')
...
def test():
    ...
    with tf.Session(graph = g2) as sess:
        ...
        graph2 = build_graph2(num_classes=FLAGS.charset_size2, top_k=71, is_train=False, is_test=True)
        saver = tf.train.Saver()
        sess.run(tf.global_variables_initializer())
        sess.run(tf.local_variables_initializer())
        if FLAGS.restore:
    ...
    for i in range(0, count):
        ...
        for j in range(0, 3755):
            ...
            pro[j] = math.log(result_67[i][a-1]+1e-50) + math.log(result_71[i][b-1]+1e-50)
