...
import pic_processing as image_preprocess
...
# tf.app.flags.DEFINE_integer('charset_size', 21003, "Choose the first `charset_size` character to conduct our experiment.")
...
# tf.app.flags.DEFINE_integer('decay_steps', 34573, "the steps to decay")
tf.app.flags.DEFINE_integer('charset_size', 94, "Choose the first `charset_size` character to conduct our experiment.")
tf.app.flags.DEFINE_integer('image_size', 112, "Needs to provide same value as in training.")
tf.app.flags.DEFINE_boolean('gray', True, "whether to change the rbg to gray")
tf.app.flags.DEFINE_integer('max_steps', 3000000000, 'the max training steps ')
tf.app.flags.DEFINE_integer('eval_steps', 1111, "the step num to eval")
tf.app.flags.DEFINE_integer('save_steps', 1111, "the steps to save")
tf.app.flags.DEFINE_integer('decay_steps', 1111, "the steps to decay")

# tf.app.flags.DEFINE_string('checkpoint_dir', './checkpoint21003/', 'the checkpoint dir')
...
# tf.app.flags.DEFINE_string('log_dir', './log21003', 'the logging dir')
tf.app.flags.DEFINE_string('checkpoint_dir', './checkpoint94/', 'the checkpoint dir')
tf.app.flags.DEFINE_string('train_data_dir', 'H:/苏统华的空间/94类字母样本for荟俨/train/', 'the train dataset dir')
tf.app.flags.DEFINE_string('test_data_dir', 'H:/苏统华的空间/94类字母样本for荟俨/test/', 'the test dataset dir')
tf.app.flags.DEFINE_string('val_data_dir', 'H:/苏统华的空间/94类字母样本for荟俨/val/', 'the val dataset dir')
tf.app.flags.DEFINE_string('log_dir', './log94', 'the logging dir')
...
def build_graph(top_k, is_train=True, num_classes=FLAGS.charset_size, is_val=True):
    images = tf.placeholder(dtype=tf.float32, shape=[None, FLAGS.image_size, FLAGS.image_size, 1], name='image_batch')
    labels = tf.placeholder(dtype=tf.int64, shape=[None], name='label_batch')
    if is_train:
        net, endpoints = lenet(images, num_classes=num_classes, is_training=True, reuse=False)
        #net, endpoints = lenet(images, num_classes=num_classes, is_training=True)
        ...
        #vali_net, vali_end_points = lenet(images, num_classes=num_classes, is_training=False)
    ...
    #num = tf.get_variable("num", [], initializer=tf.zeros_initializer())
    #rate = tf.train.exponential_decay(0.01, global_step, decay_steps=FLAGS.decay_step, decay_rate=0.97, staircase=True)
    #opt = tf.train.AdamOptimizer(learning_rate=1e-4)
    rate = tf.train.exponential_decay(2e-4, global_step, decay_steps=FLAGS.decay_steps, decay_rate=0.97,
                                      staircase=True)
    opt = tf.train.AdamOptimizer(learning_rate=rate)
    #grads = opt.compute_gradients(loss, 'conv1')
    #print(grads)
...
def train():
    ...
    with sess:
        ...
        vali_graph = build_graph(is_train=False, num_classes=FLAGS.charset_size, top_k=5, is_val=False)
        ...
        if FLAGS.restore:
            ...
            if ckpt:
                ...
                print("lenet restore from the checkpoint {0}".format(ckpt))
        ...
        while True:
            try:
                ...
                _, loss_train, acc1_train, acc10_train, train_summary, step, loss = sess.run(
                    [graph['train_op'], graph['loss'], graph['accuracy'], graph['accuracy_top_k'],
                     graph['merged_summary_op'],
                     graph['global_step'],graph['loss']], feed_dict=feed_dict)
                #logger.info("@lenet {0} loss: {1}".format(step, loss))
                global_loss += loss
                if (step % 100 == 0)&(step != 0):
                    logger.info("@lenet {0} loss: {1}".format(step, global_loss/100.0))
                    global_loss = 0.0
                ...
                if step % FLAGS.eval_steps == 0:
                    ...
                    logger.info("@lenet Accuracy of train set util {0}: {1}/{2}".format(step, allAcc1Train, allAcc10Train))
                    allAcc1Train = 0.0
                    allAcc10Train = 0.0
                    trainIter = 0.0
                    ...
                    while True:
                        try:
                            ...
                            logger.info('lenet epoch # {0}, top 1 accuracy: {1}, top 5 accuracy: {2}'.format(num, acc_top_1,
                                                                                                        acc_top_10))
                            break
                    ...
                    if maxAcc < acc[num]:
                        ...
                        maxAcc = acc[num]
                        count = 0
                        logger.info('@lenet lenet Save the ckpt of {0} step(s)/{1} epoch(es) {2}'.format(step, num,
                                                                                                    datetime.now()))
                        ...
                        count = count + 1
                        print('count=', count)
                        logger.info('@lenet Early stop num: {0}'.format(count))
                    num = num + 1
                '''
                if step > FLAGS.max_steps:
                    logger.info('@lenet Best Accuracy: {0} (Step over)'.format(maxAcc))
                    break
                if count == 10:
                    logger.info('@lenet Best Accuracy: {0} (Early stopped)'.format(maxAcc))
                    break
            except tf.errors.OutOfRangeError:
                logger.info('@lenet Best Accuracy: {0} (Out of range) step : {1}'.format(maxAcc, step))
                break
    train_writer.close()
...
def validation():
    ...
    with tf.Session() as sess:
        ...
        graph = build_graph(num_classes=FLAGS.charset_size, top_k=5, is_train=False, is_val=True)
        saver = tf.train.Saver()
        sess.run(tf.global_variables_initializer())
        sess.run(tf.local_variables_initializer())
        if FLAGS.restore:
        ...
        while True:
            try:
                ...
                logger.info("top 1 accuracy {0}, top 5 accuracy {1}".format(acc_top_1, acc_top_k))
                break
...
def inference(image):
    ...
    with tf.Session() as sess:
        ...
        graph = build_graph(num_classes=FLAGS.charset_size, top_k=13, is_train=False, is_test=True)
        saver = tf.train.Saver()
        ckpt = tf.train.latest_checkpoint(FLAGS.checkpoint_dir)
