...
tf.app.flags.DEFINE_integer('charset_size', 71, "Choose the first `charset_size` character to conduct our experiment.")
...
tf.app.flags.DEFINE_string('checkpoint_dir', './checkpoint3755_71/', 'the checkpoint dir')
...
tf.app.flags.DEFINE_string('log_dir', './log3755_71', 'the logging dir')
    ...
    with sess:
        ...
        if FLAGS.restore:
            ...
            if ckpt:
                ...
                print("lenet71 restore from the checkpoint {0}".format(ckpt))
        ...
        while True:
            try:
                ...
                if step % FLAGS.eval_steps == 0:
                    ...
                    logger.info("@lenet71 Accuracy of train set util {0}: {1}/{2}".format(step, allAcc1Train, allAcc10Train))
                    allAcc1Train = 0.0
                    allAcc10Train = 0.0
                    trainIter = 0.0
                    ...
                    while True:
                        try:
                            ...
                            logger.info('lenet71 epoch # {0}, top 1 accuracy: {1}, top 5 accuracy: {2}'.format(num, acc_top_1,
                                                                                                        acc_top_10))
                            break
                    ...
                    if maxAcc < acc[num]:
                        ...
                        maxAcc = acc[num]
                        count = 0
                        logger.info('@lenet71 lenet Save the ckpt of {0} step(s)/{1} epoch(es) {2}'.format(step, num,
                                                                                                    datetime.now()))
                        ...
                        count = count + 1
                        print('count=', count)
                        logger.info('@lenet71 Early stop num: {0}'.format(count))
                    num = num + 1
                if step > FLAGS.max_steps:
                    logger.info('@lenet71 Best Accuracy: {0} (Step over)'.format(maxAcc))
                    break
                if count == 10:
                    logger.info('@lenet71 Best Accuracy: {0} (Early stopped)'.format(maxAcc))
                    break
            except tf.errors.OutOfRangeError:
                logger.info('@lenet71 Best Accuracy: {0} (Out of range) step : {1}'.format(maxAcc, step))
                break
    train_writer.close()
