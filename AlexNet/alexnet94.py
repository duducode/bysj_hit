...
tf.app.flags.DEFINE_string('checkpoint_dir', './checkpoint94_11/', 'the checkpoint dir')
...
tf.app.flags.DEFINE_string('log_dir', './log94_11', 'the logging dir')
...
def build_graph(top_k, is_train=True, num_classes=FLAGS.charset_size, is_val=False):
    images = tf.placeholder(dtype=tf.float32, shape=[None, FLAGS.image_size, FLAGS.image_size, 1], name='image_batch')
    labels = tf.placeholder(dtype=tf.int64, shape=[None], name='label_batch')
    if is_train:
        net, endpoints = alexnet_v2(images, num_classes=num_classes, is_training=True, reuse=False)
        ...
        if is_val==True:
            ...
            vali_net, vali_end_points = alexnet_v2(images, num_classes=num_classes, is_training=False, reuse=True)
    ...
    with sess:
        ...
        if FLAGS.restore:
            ...
            if ckpt:
                ...
                print("Alexnet3755 restore from the checkpoint {0}".format(ckpt))
        ...
        while True:
            try:
                ...
                train_labels_batch = train_labels_batch % 11
                ...
                #     global_loss /= 100
                #     logger.info("@alexnet {0} loss: {1}".format(step, global_loss))
                #     global_loss = 0.0
                if step % FLAGS.eval_steps == 0:
                    val_images, val_labels = get_batch_val(dirpath=FLAGS.val_data_dir, lb='val')
                    val_labels = val_labels % 11
                    allAcc1Train = allAcc1Train / trainIter
                    allAcc10Train = allAcc10Train / trainIter
                    logger.info("@Alexnet3755 Accuracy of train set util {0}: {1}/{2}".format(step, allAcc1Train, allAcc10Train))
                    allAcc1Train = 0.0
                    allAcc10Train = 0.0
                    trainIter = 0.0
                    ...
                    while True:
                        try:
                            ...
                            logger.info('Alexnet3755 epoch # {0}, top 1 accuracy: {1}, top 5 accuracy: {2}'.format(num, acc_top_1,
                                                                                                        acc_top_10))
                            break
                    ...
                    if maxAcc < acc[num]:
                        ...
                        maxAcc = acc[num]
                        count = 0
                        logger.info('@Alexnet3755 Save the ckpt of {0} step(s)/{1} epoch(es)'.format(step, num))
                        saver.save(sess, os.path.join(FLAGS.checkpoint_dir, 'my-model'),
                                   global_step=graph['global_step'])
                    else:
                        count = count + 1
                        print('count=', count)
                        logger.info('@Alexnet3755 Early stop num: {0}'.format(count))
                    num = num + 1
                if step > FLAGS.max_steps:
                    logger.info('@Alexnet3755 Best Accuracy: {0} (Step over)'.format(maxAcc))
                    break
                if count == 10:
                    logger.info('@Alexnet3755 Best Accuracy: {0} (Early stopped)'.format(maxAcc))
                    break
            except tf.errors.OutOfRangeError:
                logger.info('@Alexnet3755 Best Accuracy: {0} (Out of range) step : {1}'.format(maxAcc, step))
                break
    train_writer.close()
