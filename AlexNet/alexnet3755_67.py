    ...
    with sess:
        ...
        if FLAGS.restore:
            ...
            if ckpt:
                ...
                print("Alexnet3755_67 restore from the checkpoint {0}".format(ckpt))
        ...
        while True:
            try:
                ...
                if step % FLAGS.eval_steps == 0:
                    ...
                    logger.info("@Alexnet3755_67 Accuracy of train set util {0}: {1}/{2}".format(step, allAcc1Train, allAcc10Train))
                    allAcc1Train = 0.0
                    allAcc10Train = 0.0
                    trainIter = 0.0
                    ...
                    while True:
                        try:
                            ...
                            logger.info('Alexnet3755_67 epoch # {0}, top 1 accuracy: {1}, top 5 accuracy: {2}'.format(num, acc_top_1,
                                                                                                        acc_top_10))
                            break
                    ...
                    if maxAcc < acc[num]:
                        ...
                        maxAcc = acc[num]
                        count = 0
                        logger.info('@Alexnet3755_67 Save the ckpt of {0} step(s)/{1} epoch(es)'.format(step, num))
                        saver.save(sess, os.path.join(FLAGS.checkpoint_dir, 'my-model'),
                                   global_step=graph['global_step'])
                    else:
                        count = count + 1
                        print('count=', count)
                        logger.info('@Alexnet3755_67 Early stop num: {0}'.format(count))
                    num = num + 1
                if step > FLAGS.max_steps:
                    logger.info('@Alexnet3755_67 Best Accuracy: {0} (Step over)'.format(maxAcc))
                    break
                if count == 10:
                    logger.info('@Alexnet3755_67 Best Accuracy: {0} (Early stopped)'.format(maxAcc))
                    break
            except tf.errors.OutOfRangeError:
                logger.info('@Alexnet3755_67 Best Accuracy: {0} (Out of range) step : {1}'.format(maxAcc, step))
                break
    train_writer.close()
