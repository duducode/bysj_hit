...
tf.app.flags.DEFINE_string('train_data_dir', 'H:/苏统华的空间/3755类汉字样本for荟俨/test/', 'the train dataset dir')
tf.app.flags.DEFINE_string('test_data_dir', 'H:/苏统华的空间/3755类汉字样本for荟俨/test/', 'the test dataset dir')
tf.app.flags.DEFINE_string('val_data_dir', 'H:/苏统华的空间/3755类汉字样本for荟俨/test/', 'the val dataset dir')
...
def train():
    ...
    global step
    with sess:
        train_images, train_labels = get_batch_train(dirpath=FLAGS.train_data_dir,  lb='test')
        ...
        while True:
            try:
                ...
                #train_labels_batch = train_labels_batch % 67
                for m in range(0, 255):
                    images = train_images_batch[m]
                    h, w, c = images.shape
                    print(images.shape)
                    assert c == 1
                    images = images.reshape(h, w)
                    plt.imshow(images, cmap='gray')
                    print(train_labels_batch[m])
                    plt.show()
